...
export function registerAddSlideTool(server: McpServer, container: ServiceContainer): void {
  server.registerTool(
    'add_slide',
    {
      title: 'Add Slide',
      description:
        'Add a new slide to a deck. The slide HTML must declare .slide dimensions that match the DECK FORMAT: 1920×1080 for slides_16_9 (default), 1240×1754 for print_a4_portrait, 1275×1650 for print_letter_portrait. .slide MUST also carry "position: relative" (absolute descendants need it as their containing block — a common pitfall that surfaces as false-looking overflow errors) and "padding: var(--space-safe-area)". Declare "html, body { margin: 0 }" explicitly (the * selector does not override the browser UA body margin). See pengui://docs/slide-format for the canonical template + Common Pitfalls section, and pengui://docs/print-mode for print decks. Returns the slide ID, position, slide count, and validation results.',
